  ...
  return (
    <div style={{ padding: "0 8px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginLeft: 140, marginTop: 8 }}>
        {ticks.map((t, i) => (
          <div key={i} style={{ fontSize: 11, color: "rgba(232,228,220,0.3)" }}>{fmtFull(t)}</div>
        ))}
      </div>
    </div>
  );
...
function render(){
  ...
  document.getElementById("monthlyVal").textContent="월 "+fmtW(Math.round(totS/12));
  ...
  let hdr="<thead><tr><th class='al'>"+tab.label.replace("별","")+"</th><th class='ar'>인원</th><th class='ar'>인원 비율</th><th class='ar'>연봉 합계</th><th class='ar'>월급 합계</th><th class='ar'>연봉 비율</th>"+(incRet?"<th class='ar'>리텐션/사이닝</th>":"")+(incPerf?"<th class='ar'>성과</th>":"")+"<th class='ar'>평균 연봉</th><th class='ar'>총합</th></tr></thead>";
  let body="<tbody>"+d.map((x,idx)=>{return"<tr><td class='al fw'>"+x.name+"</td><td class='ar dim'>"+x.cnt+"명</td><td class='ar dim'>"+(totCnt?(x.cnt/totCnt*100).toFixed(1):0)+"%</td><td class='ar purple'>"+fmtM(x.base)+"</td><td class='ar purple' style='opacity:0.6'>"+fmtM(Math.round(x.total/12))+"</td><td class='ar dim'>"+(totS?(x.total/totS*100).toFixed(1):0)+"%</td>"+(incRet?"<td class='ar lpurple'>"+(x.ret>0?fmtM(x.ret):"-")+"</td>":"")+(incPerf?"<td class='ar lpurple'>"+(x.perf>0?fmtM(x.perf):"-")+"</td>":"")+"<td class='ar dim'>"+fmtM(x.avg)+"</td><td class='ar white'>"+fmtM(x.total)+"</td></tr>"}).join("")+"</tbody>";
        ...
        {data.length === 0 ? (
          ...
          <>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 28, flexWrap: "wrap", gap: 16 }}>
              ...
              <div style={{ textAlign: "right" }}><div style={{ fontSize: 12, color: "rgba(232,228,220,0.4)" }}>총 연봉 합계</div><div style={{ fontSize: 26, fontWeight: 800, color: "#fff" }}>{fmt(totalSalary)}</div>
<div style={{ fontSize: 12, color: "rgba(232,228,220,0.35)", marginTop: 2 }}>월 {fmt(Math.round(totalSalary / 12))}</div></div>
            </div>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                  <thead>
                    <tr style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                      {[currentTab.label.replace("별", ""), "인원", "인원 비율", "연봉 합계", "월급 합계", "연봉 비율", ...(incRet ? ["리텐션/사이닝"] : []), ...(incPerf ? ["성과"] : []), "평균 연봉", "총합"].map((h, i) => (
                    ...
                    {chartData.map((d, i) => {
                      return (
                        <tr key={i} style={{ borderBottom: "1px solid rgba(255,255,255,0.03)" }}>
                          ...
                          <td style={{ padding: "12px 16px", textAlign: "right", color: "rgba(168,155,255,0.6)" }}>{fmtM(Math.round(d.total / 12))}</td>
