...
- **본부별(dept) 탭**:
  - 막대 차트·표·추이: 모든 직원 포함
  - **분포도(RangeChart): 직급=`이사` 개인은 제외** (개별 임원 연봉이 분포 양극에 노출되는 것 방지)
- **조직별(team) 탭**: CEO/CTO/COO/CFO 그룹 제외
- **직급별(rank) 탭**: 이사 그룹 제외
- **직무별(job) 탭**: 전체 포함

> 참고: 의뢰자가 명시. "C-lv은 분쟁의 소지가 있어 조직 표시 자체를 안 함"이라 CSV에 C-lv 본부가 없을 수 있음. 코드는 C-lv가 본부 컬럼에 들어와도 정상 동작하나, 실제 데이터에는 없는 상태.
