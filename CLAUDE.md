...
### 8.1. 탭별 필터링 (의뢰자 결정사항)
- **헤더 전사 총계 (전체 인원/총 연봉 합계)**: 이사 포함 **모두 포함** (대규모 합계라 개인 식별 불가 → 총 인건비 정확성 유지). 헤더에 "임원(이사) 포함 · 본부별 상세는 이사 제외" 안내 표기.
- **모든 상세 탭(본부별/조직별/직급별/직무별)**: 합계·인원·비율·평균·분포도 **전부 직급=`이사` 제외** (2026-06-22 변경)
  - **이유 (이사 연봉 역산 누수 차단)**: 종전에는 본부별 표가 "합계(이사 포함)"와 "평균(이사 제외)"을 같은 행에 노출 → 단일 본부에 이사가 1명만 있으면 `합계 − 평균×(인원−이사수)`로 그 이사 연봉이 **정확히 역산**됐음. (CTO가 CEO 연봉을 알게 되면 안 된다는 우려가 그대로 현실화.) 이사를 모든 셀에서 빼서 차단.
  - 부수 효과: `합계 ÷ 인원 = 평균`이 정확히 일치(기준 통일). 분포도·인상률도 이미 이사 제외였으므로 전 지표 일관.
  - 잔여 위험: 헤더합계(이사 포함) − Σ본부합계(이사 제외) = Σ이사+미지정. 이사 ≥2명이면 개별 분리 불가. **이사가 전사 1명만 남으면** 헤더도 이사 제외로 전환 필요.
- **조직별(team) 탭**: 위 이사 제외 + 추가로 CEO/CTO/COO/CFO 그룹 제외

> 참고: 의뢰자 명시. "C-lv은 분쟁의 소지가 있어 조직 표시 자체를 안 함"이라 CSV에 C-lv 본부가 없을 수 있음. 실제 데이터 직급은 `이사/팀장/매니저/미지정`만 존재(별도 CEO/CTO 직급 없음).

### 8.2. 코드 위치 (`lib/aggregate.js`)
```js
const CLV_TEAMS = ["CEO", "CTO", "COO", "CFO"];
const TAB_EXCLUDE = {           // 탭별 "추가" 제외 (이사 제외는 아래 tabData에서 공통 적용)
  rank: (d) => d.rank === "이사",
  team: (d) => CLV_TEAMS.includes(d.team),
};
// 탭 루프: 이사는 모든 상세 탭에서 공통 제외 + team은 CLV 추가 제외
// const tabData = data.filter((d) => d.rank !== "이사" && !(excludeFn && excludeFn(d)));
```
...
*마지막 업데이트: 2026-06-22 — (1) 인상률 산식을 "기간 내 인상자 개인별 인상률 평균"(B안)으로 변경(snapshotAt/pastRaiseSet 폐기 → 본부별 `-%` 혼란 제거, `이전 계약금액` 없는 인원 제외). (2) 기간을 달력 기준(분기=1~3월/4~6월…, 반기, 연)으로 책정, `raisePeriodLabel`/`raiseComparePhrase` 추가. (3) 상세 표·차트의 합계·인원·비율·평균에서 **이사 직급 전부 제외**(이사 연봉 역산 누수 차단, 합계÷인원=평균 일치). 헤더 전사 총계만 이사 포함 + "임원(이사) 포함" 안내 문구 추가. 추가 작업 시 이 줄 갱신.*
