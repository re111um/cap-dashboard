...
const RANK_ORDER = ["이사", "팀장", "매니저"];
const CLV_TEAMS = ["CEO", "CTO", "COO", "CFO"];

function isClv(d) {
  return CLV_TEAMS.includes(d.team);
}
const TABS = [
...
export function computeAll(rawData, { incRet, incPerf, hideClv, asOfDate }) {
  let data = hideClv ? rawData.filter((d) => !isClv(d)) : rawData;
  ...
  for (const tab of TABS) {
    ...
    // 모든 탭의 분포도에서 C-lv 항상 제외 (hideClv 토글과 무관)
    const distSource = data.filter((d) => !isClv(d));

    data.forEach((d) => {
      const k = d[tab.field] || "미지정";
