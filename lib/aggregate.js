...
export function computeAll(rawData, { incRet, incPerf, asOfMonth, raisePeriodMonths = 12 }) {
  ...
  for (const tab of TABS) {
    ...
    // 탭별 제외 규칙 (상세 표·차트):
    // - 임원(이사) 직급은 모든 탭에서 제외 → 개인 연봉 역산 방지 + 합계÷인원=평균 일관성
    //   (단일 본부에 이사 1명만 있어도 "합계(이사 포함) − 평균(이사 제외)×인원"으로
    //    역산되던 누수를 차단)
    // - team 탭은 추가로 C-lv(CEO/CTO/COO/CFO) 제외
    // - 전사 총계(헤더 totalCount/totalSalary)에는 이사가 포함됨 (대규모 합계라 개인 식별 불가)
    ...
    tabData.forEach((d) => {
      ...
      // 분포도 (이사는 tabData에서 이미 제외됨)
      if (!groups[k]) groups[k] = [];
      groups[k].push(d.salary + (incRet ? d.incRet : 0) + (incPerf ? d.inc26 : 0));
    });

    let aggArr = Object.values(agg).map((x) => {
      const total = x.baseSalary + (incRet ? x.retInc : 0) + (incPerf ? x.perfInc : 0);
      return {
        ...x,
        total,
        avg: x.count ? Math.round(total / x.count) : 0, // 이사 제외 → 합계÷인원과 일치
