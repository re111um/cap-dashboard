...
export function computeAll(rawData, { incRet, incPerf, hideClv, asOfDate }) {
  let data = hideClv ? rawData.filter((d) => d.dept !== "C-lv") : rawData;
  ...
  for (const tab of TABS) {
    ...
    // 본부별·직급별 분포도는 C-lv 항상 제외 (hideClv 토글과 무관)
    const distSource = (tab.key === "rank" || tab.key === "dept")
      ? data.filter((d) => d.dept !== "C-lv")
      : data;
