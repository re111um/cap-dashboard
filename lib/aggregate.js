...
/**
 * 서버 사이드에서 모든 집계를 수행하고 결과만 반환
 * RAW 데이터는 절대 클라이언트에 전달되지 않음
 */
export function computeAll(rawData, { incRet, incPerf, hideClv }) {
  const data = hideClv ? rawData.filter((d) => d.dept !== "C-lv") : rawData;
